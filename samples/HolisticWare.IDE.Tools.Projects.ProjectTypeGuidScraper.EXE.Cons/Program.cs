...
namespace HolisticWare.IDE.Tools.Projects.ProjectTypeGuidScraper.EXE
{
	class Program
	{
		...
		private static void SerializationWarmUp()
		{
			...
			s = new System.Xml.Serialization.XmlSerializer((typeof(Project)));
		...
		static protected async Task<Project> ReadProjectAsync(string filename)
		{
            ...
            try
            {
                string project_file_content = null;
                using (System.IO.StreamReader reader = new System.IO.StreamReader(fs))
				{
                    project = (Project)serializer.Deserialize(fs);

                    project_file_content = await reader.ReadToEndAsync();
